    ...
    <>
      <div className={styles.post_page_container} >
        <div className={styles.post_img_container}>
          {posts.map(({ userId,postsIndex,name,url }, index) => users[users.findIndex(el =>el.id === userId)].isAuth ? null : <div className={styles.posts_container} key={index}><div className={styles.user_container}>< img className={styles.user_img} src={users[users.findIndex(el =>el.id === userId)].url} alt="user"/><h3 className={styles.user_name}>{users[users.findIndex(el =>el.id === userId)].name}</h3></div><img className={styles.posts_img} src={url} alt="post" />{postssArr[users.findIndex(el =>el.id === userId)].posts[postsIndex].comments.map((el,index) => el.isVisible? <div className={styles.comments_container} key={index}><h3 className={styles.comment_user_name}>{users[users.findIndex(el => el.id === userId)].name}</h3><h3 className={styles.comment_user_text}>{el.text}</h3></div> : null )}<button onClick={  () =>{
         dispatch(setCommentsAC())
            dispatch(showCommentsAC({userIndex:users.findIndex(el =>el.id ===userId),index:postsIndex,comments:comments}))
         
        dispatch({type:setProducts})}}>Show more</button><span className={styles.posts_span}>Likes:{productsArr[users.findIndex(el =>el.id === userId)].posts[postsIndex].likes}</span><svg onClick={async() => {
            await dispatch(setCounterAC())
            dispatch(incrementLikesAC({ index: postsIndex, userIndex: users.findIndex(el =>el.id === userId) ,counter:productsArr}))
          ...
          <CommentsForm index={postsIndex} userIndex={users.findIndex(el => el.id === userId)} userId ={userId} /></div>)
